...
def compIsValidWord(word, hand):
    """ Similar to the isValidWord implementation in ps4a that omits
        the check in wordlist - the word has been chosen from there
    :param word:
    :param hand:
    :return:
    """
    try:
        result_hand = hand.copy()
        for letter in word:
            result_hand[letter] -= 1
    except KeyError as e:
        return False

    if -1 in result_hand.values():
        return False
    else:
        return True
...
def playGame(wordList):
    ...
    while True:
        next_step = str(raw_input('Enter n to deal a new hand, r to replay the last hand, or e to end game: '))
        print

        if hand == {} and next_step == 'r':
            print('You have not played a hand yet. Please play a new hand first!')
            print

        elif next_step == 'n':
            hand = dealHand(HAND_SIZE)
            choose_player = str(raw_input('Enter u to have yourself play, c to have the computer play: '))
            while choose_player not in ['u', 'c']:
