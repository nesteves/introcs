...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a_node == b_node:
            return -1, 0
        elif a_node.get_parent() == b_node or b_node.get_parent() == a_node:
            return -1, 1

        la = [a_node]
        lb = [b_node]

        while la[-1] not in lb and lb[-1].name not in la:
            if la[-1].get_parent() is not None:
                la.append(la[-1].get_parent())
            if lb[-1].get_parent() is not None:
                lb.append(lb[-1].get_parent())

        print [i.name for i in la]
        print [i.name for i in lb]

        if la[-1] == lb[-1]:
            cousin_dist = min(len(la), len(lb)) - 2
            removed_dist = abs(len(la) - len(lb))
        else:
            if la.count(lb[-1]) > 0:
                used_length = len(la[:la.index(lb[-1])]) + 1
                cousin_dist = min(used_length, len(lb)) - 2
                removed_dist = abs(used_length - len(lb))
            else:
                used_length = len(lb[:lb.index(la[-1])]) + 1
                cousin_dist = min(used_length, len(la)) - 2
                removed_dist = abs(used_length - len(la))

        if la[0] in lb or lb[0] in la:
            cousin_dist = -1

        return cousin_dist, removed_dist
...
if __name__ == '__main__':
    ...
    t, r = f.cousin("j", "d")
    print "'j' is a", words[t], "cousin", r, "removed from 'd'"
